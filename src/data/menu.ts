// Tea°ut 메뉴 데이터 (단일 소스)
// 가격: 매장 메뉴판(public/paper/tea_paper) 기준. HOT/ICE 동일 단가(단일 표기).
// 카드/모달이 동일 객체를 사용한다.

export interface MenuPrice {
  label: string; // HOT/ICE / ICE / HOT
  price: string; // "5,500원"
}
...
// HOT/ICE 동일 단가 헬퍼
const both = (price: string): MenuPrice[] => [{ label: "HOT/ICE", price }];
const ice = (price: string): MenuPrice[] => [{ label: "ICE", price }];
const hot = (price: string): MenuPrice[] => [{ label: "HOT", price }];
    ...
    items: [
      {
        ...
        desc: "5가지 차 베이스 중 원하는 차를 선택할 수 있는 밀크티입니다. 홍차·계화·설국·호지 라떼는 7,600원, 아포라떼는 9,400원입니다. 계화 · 설국 · 호지는 디카페인으로 저녁에도 부담 없이 즐기실 수 있습니다.",
        cardExtra: "아포라떼 9,400원 · 계화/설국/호지 디카페인",
        extra: "오트밀크 변경 · 딸기·복숭아청 추가 각 +500원",
        prices: both("7,600원"),
        featured: true,
    ...
    items: [
      {
        ...
        note: "아포 · 계화 · 설국 중 선택",
        desc: "3가지 차 베이스 중 원하는 차를 골라 시원한 탄산과 함께 즐기는 에이드입니다. 계화·설국 에이드는 7,600원, 아포 에이드는 9,400원이며, 계화와 설국 베이스는 디카페인입니다.",
        cardExtra: "아포 에이드 9,400원 · 계화/설국 디카페인",
        prices: ice("7,600원"),
        tag: "ICE ONLY",
        tagClass: "tag-ice",
    ...
    items: [
      {
        ...
        note: "레몬티로 상큼하게!",
        desc: "계화의 달콤한 꽃향과 레몬의 상큼함이 만난 따뜻한 차음료입니다.",
        prices: hot("7,000원"),
    ...
    items: [
      {
        ...
        desc: "티읕에서 직접 선별한 원두로 정성껏 브루잉한 커피입니다. HOT과 ICE 모두 즐길 수 있으며, 풍부한 아로마와 산뜻한 산미의 조화로움을 한 잔에 담았습니다.",
        prices: both("7,000원"),
        catLabel: "COFFEE · 커피",
      },
    ],
